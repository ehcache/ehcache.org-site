...
In Ehcache, the `MemoryStore` may be limited in size (see [How to Size Caches](/documentation/configuration/cache-size) for more information). When the store gets full, elements are evicted. The eviction algorithms in Ehcache determine which
...
the evicted element will overflow to disk (is *flushed* to disk); otherwise it will be removed. The `DiskStore` size by default is unbounded. But a maximum size can be set (see [Sizing Caches](/documentation/configuration/cache-size) for more information). If the `DiskStore` is full, then adding an element
