...
they are mandatory. See the [Configuration](http://ehcache.org/documentation/configuration/configuration) page.
...
While disk persistence between restarts is a feature of the DiskStore only, you can get the same behavior for a memory-only cache
by setting up a cache with `maxEntriesLocalHeap` set to a value appropriate for your environment and `diskPersistent` set to true.
...
maxEntriesLocalHeap is the maximum number of elements in the local L1 store.
...
overflowToDisk normally controls whether to overflow to the DiskStore. This is ignored when using Terracotta because the DiskStore
...
#### What is the difference between TTL, TTI, and eternal?
 ...
 For more information, see [Setting Expiration](http://ehcache.org/documentation/configuration/data-life#24283).
...
See this [recipe](/documentation/recipes/cachenull).
...
evicted. If the overflowToDisk cache attribute is true, the LRU element is flushed asynchronously to the DiskStore.

#### Why can't I run multiple applications using Ehcache on one machine?
Because of an RMI bug, in JDKs before JDK1.5 such as JDK1.4.2, Ehcache is limited to one CacheManager per virtual machine operating in a replicated cache topology.
...
#### How does the DCV2 eviction algorithm work?
...
#### Does Ehcache run on JDK 1.3 or JDK 1.4?

Older versions run on 1.3. Ehcache 1.5 runs on 1.4. Ehcache 1.6 required JDK 1.5. Current versions of Ehcache can be run with JDK 1.6.
...
Version 1.6 is compatible. See [Google App Engine Caching](http://ehcache.org/documentation/integrations/googleappengine).
...
See [Tomcat Issues and Best Practices](http://ehcache.org/documentation/integrations/tomcat).
...
they are used.
 If the CacheManager does not get shut down, it should not be a problem.
There is a shutdown hook which calls the shutdown on JVM exit. This is
explained in the documentation [here](http://ehcache.org/documentation/operations/shutdown).
...
Statistics should be enabled when using the [Ehcache Monitor](http://ehcache.org/documentation/operations/monitor).

#### How do I detect deadlocks in Ehcache?
Ehcache does not experience deadlocks. However, deadlocks in your application code can be detected with certain tools, such as [JConsole](/documentation/operations/jmx#JConsole-Example).
...
#### Why did a crashed standalone Ehcache node using diskstore not come up with all data intact?
Persistence is provided by standalone Ehcache nodes that are using the diskstore *and* are shut down
*cleanly*. Nodes that shut down due to a crash or unexpectedly cannot execute the shutdown procedure that persists the cache keys (all of which are in heap).

For crash-resistant persistence, use distributed cache, which is backed by the Terracotta Server Array.
...
#### Why doesn't overflowToDisk work with Terracotta clustered caches?
...
3. Tune the [concurrency of the cache](/documentation/configuration/distributed-cache-configuration#70873). It may be set too high for a small data set.
...
* If you only want one instance, use the singleton creation methods, i.e., `CacheManager.getInstance()`. In Hibernate, there is a special provider for this called
 ...
 See [Hibernate](http://ehcache.org/documentation/integrations/hibernate).
