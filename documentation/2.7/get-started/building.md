...
These instructions work for each of the [modules](http://ehcache.org/modules), except for `ehcache-jmsreplication`, which requires installation of a message queue. See [Replicated Caching using JMS](/documentation/2.7/replication/jms-replicated-caching) for details.
...
which provides a choice of concrete logging implementation. See the page on [Logging](/documentation/2.7/operations/logging) for configuration details.
