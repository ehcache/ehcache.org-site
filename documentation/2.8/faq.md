...
they are mandatory. See the [Configuration](/documentation/2.8/configuration/configuration.html) page.
...
reinsert the value.
 The [UpdatingCacheEntryFactory](/apidocs/2.8.5/net/sf/ehcache/constructs/blocking/UpdatingCacheEntryFactory.html)
...
#### What is the difference between TTL, TTI, and eternal?
 ...
 For more information, see [Setting Expiration](/documentation/2.8/configuration/data-life.html#24283).
...
See this [recipe](/documentation/2.8/recipes/cachenull.html).
...
Standalone Ehcache supports LRU, LFU and FIFO eviction strategies, as well as custom evictors. For more information, refer to [Cache Eviction Algorthims](/documentation/2.8/apis/cache-eviction-algorithms.html).
...
Version 1.6 is compatible. See [Google App Engine Caching](/documentation/2.8/integrations/googleappengine.html).
...
See [Tomcat Issues and Best Practices](/documentation/2.8/integrations/tomcat.html).
...
#### How do you get an element without affecting statistics?
 Use the [Cache.getQuiet()](/apidocs/2.8.4/net/sf/ehcache/Cache.html#getQuiet%28java.io.Serializable%29) method. It returns an element without updating statistics.
...
explained in the documentation [here](/documentation/2.8/operations/shutdown.html).
...
* In the [Terracotta Developers Console](http://terracotta.org/documentation/2.8/terracotta-tools/dev-console.html).

    To function, certain features in the Developers Console require statistics to be enabled.

Statistics should be enabled when using the [Ehcache Monitor](/documentation/2.8/operations/monitor.html).

#### How do I detect deadlocks in Ehcache?
Ehcache does not experience deadlocks. However, deadlocks in your application code can be detected with certain tools, such as [JConsole](/documentation/2.8/operations/jmx.html#JConsole-Example).

### Replicated Caching

#### How can I see if replicated caching is working?
You should see the listener port open on each server.
You can use the replicated cache debug tool to see what is going on. (See [Remote Network Debugging and Monitoring for Distributed Caches](http://www.ehcache.org/documentation/2.8/operations/remotedebugger.html)).
...
3. Tune the [concurrency of the cache](/documentation/2.8/configuration/distributed-cache-configuration.html#70873). It may be set too high for a small data set.
...
* If you only want one instance, use the singleton creation methods, i.e., `CacheManager.getInstance()`. In Hibernate, there is a special provider for this called
 ...
 See [Hibernate](/documentation/2.8/integrations/hibernate.html).
